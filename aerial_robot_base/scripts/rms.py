...
class TreeDb:
        ...
        def __init__(self, pos_x, pos_y, radius, marker_id):
                self.pos_x:float = pos_x + TreeDb.shift_x
                self.pos_y:float = pos_y + TreeDb.shift_y
...
def cb(data):
        ...
        if start_flag == True:
                ...
                for i, tree_db in enumerate(tree_db_list):
...
if __name__=="__main__":
        ...
        tree_db_list = []
        tree_db_list.append(TreeDb(1.2,0.5,0.25,4))
        tree_db_list.append(TreeDb(2.5,-0.9,0.25,0))
        tree_squared_errors_sum_list = [[0] * 3 for _ in range(len(tree_db_list))]
        ...
        try:
                while(True):
                        ...
                        if key == 'h':
                                ...
                                for j, tree_squared_errors_sum in enumerate(tree_squared_errors_sum_list):
                                        rms = [0] * 3
                                        if pose_cnt > 0:
                                                rms = [math.sqrt(i / pose_cnt) for i in tree_squared_errors_sum]
