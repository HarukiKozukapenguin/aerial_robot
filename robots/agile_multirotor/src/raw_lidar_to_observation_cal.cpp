...
#include <raw_lidar_to_observation_cal.h>
...
ObstacleCalculator::ObstacleCalculator(ros::NodeHandle nh, ros::NodeHandle pnh)
  ...
  scan_sub_ = nh_.subscribe("/" + quad_name + "/scan", 1,
...
void ObstacleCalculator::ScanCallback(const sensor_msgs::LaserScan::ConstPtr &msg){
    ...
    float scan_angle_min_ = msg->angle_min;
    // scan_angle_max_ = scan->angle_max;
    float scan_angle_increment = msg->angle_increment;
    for (size_t i=0; i< scan_data.size(); i++) {
      float range = scan_data[i];
      float theta = scan_angle_min_ + scan_angle_increment*i;
      Eigen::Vector3d body_tree_pos(range*std::cos(theta), range*std::sin(theta), 0);
      Eigen::Vector3d tree_pos = R * body_tree_pos + quadrotor_pos_;
      // tree_pos(0) = R * range*std::cos(theta) + quadrotor_pos_(0);
      // tree_pos(1) = R * range*std::sin(theta) + quadrotor_pos_(1);
      // tree_pos(2) = 0.0;
...
void ObstacleCalculator::CalculatorCallback(
  ...
  tf::pointMsgToEigen(msg->pose.pose.position, quadrotor_pos_);
  ...
  if (!from_hokuyo_){
    ...
    for (; it_pos != positions_.end() && it_vel != velocities_.end(); ++it_pos, ++it_vel) {
      ...
      marker.scale.y = 0.25;
      marker.scale.z = 2.0;
      Eigen::Vector3d converted_pos = R_T * (tree_pos - quadrotor_pos_);
      converted_positions.push_back(converted_pos);
      Eigen::Vector2d converted_pos_2d = {(tree_pos - quadrotor_pos_)[0], (tree_pos - quadrotor_pos_)[1]}; //world coordinate
  ...
  Vector<Vision::Theta_Cuts> sphericalboxel =
      getsphericalboxel<Vector<Vision::Theta_Cuts>>(converted_positions, poll_x, poll_y, poll_z, theta_list_, quadrotor_pos_);

  Vector<3> vel_2d = {vel[0], vel[1], 0};
  Vector<3> body_vel = R_T * vel_2d;
  Vector<Vision::ACC_Theta_Cuts> acc_sphericalboxel =
    getsphericalboxel<Vector<Vision::ACC_Theta_Cuts>>(converted_positions, poll_x, poll_y, poll_z, acc_theta_list_, quadrotor_pos_, body_vel);

  get_common_sphericalboxel(converted_positions, poll_x, poll_y, poll_z, R_T, vel, omega, quadrotor_pos_);
