...
int main(int argc, char** argv)
{
  ...
  float transition_pos_x = 0.0, transition_pos_y = -0.2, transition_pos_z = -0.13;
  ...
  while (ros::ok())
  {
    ...
    marker.pose.position.z = -.3+transition_pos_z;
    marker.scale.x = 0.4;
    marker.scale.y = 0.4;
    marker.scale.z = 2.0;
    ...
    marker.id = 2;
    roll = 1.57, pitch = 3.14*2/3, yaw = 0.0;
    q = Eigen::AngleAxis<float>(roll, Vector<3>::UnitX()) *
    Eigen::AngleAxis<float>(pitch, Vector<3>::UnitY()) *
    Eigen::AngleAxis<float>(yaw, Vector<3>::UnitZ())*transition_q;
    marker.pose.orientation.x = q.x();
    ...
    marker.pose.orientation.z = q.z();
    marker.pose.orientation.w = q.w();
    marker.pose.position.x = -0.2+transition_pos_x;
    marker.pose.position.y = -0.3+transition_pos_y;
    marker.pose.position.z = -.2+transition_pos_z;
    marker.scale.x = 0.2;
    marker.scale.y = 0.2;
    marker.scale.z = .6;
